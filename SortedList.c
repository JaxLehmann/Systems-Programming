...
// Get node value
int GetNodeData(IntNode *thisNode) { return thisNode->dataVal; }

// Get pointer to next node
IntNode *GetNext(IntNode *thisNode) { return thisNode->nextNodePtr; }
...
bool IsSorted(IntNode *headNode) {
  ...
  /* Type your code here. */
  while (headNode->nextNodePtr) {
    if (headNode->dataVal > headNode->nextNodePtr->dataVal) {
      return false;
    } else {
